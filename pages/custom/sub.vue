  ...
  <view class="wrapper">
    ...
    <view class="content">
      <view class="item" v-for="item in showList" :key="item.pkId" @click="openPop(item)" >
        <u-icon name="../../static/image/cussub.png" class="iconfont" size="20"></u-icon>
        <view class="item-content">
          ...
          <view class="types">联系人：{{ item.orgLinkMan }}</view>
        </view>
      </view>
    </view>
    <view class="pdb"></view>
    <view class="btn"  v-if="$auth('custom:sub:add')" @click="go(1)">新增</view>
      ...
      <view class="popup detil-pop">
        ...
        <view class="detil-pop-footer footer">
          <u-button
            ...
            type="default"
            text="删除"
            v-if="$auth('custom:sub:delete')"
            @click="delCus"
          ></u-button>
          <u-button  
            class="btns"
            v-if="nowClick.linkStatus==1 && $auth('custom:sub:binding')"
            type="warning"
            text="解绑"
            @click="relieveLink"
          ></u-button>
          <u-button  
            class="btns"
            v-if="!nowClick.linkStatus && $auth('custom:sub:binding')"
            type="success"
            text="绑定"
            @click="openLinkPop"
          ></u-button>
          <u-button
          v-if="$auth('custom:sub:update')"
            class="btns"
            type="primary"
            text="编辑"
            @click="go(2)"
