<template>
  <view class="wrapper">
    ...
    <view class="sticky">
      <view class="search">
        <u-input placeholder="请输入供应商名称" border="none" v-model="name" maxlength="25">
    ...
    <u-popup :show="showPop" :round="10">
      <view class="popup detil-pop">
        ...
        <view class="detil-pop-footer footer">
          ...
          <u-button
          v-if="$auth('custom:supply:delete')"
            class="btns cancle"
            type="default"
            text="删除"
            @click="delCus"
          ...
          <u-button
            class="btns"
            v-if="!!nowClick.linkStatus && $auth('custom:supply:binding')"
            type="warning"
            text="解绑"
            @click="relieveLink"
          ></u-button>
          ...
          ></u-button>
          <u-button
          v-if="$auth('custom:supply:update')"
            class="btns"
            type="primary"
            text="编辑"
            @click="go(2)"
...
export default {
  ...
  methods: {
    ...
    openPop(item) {
      this.nowClick = item;
      this.nowClick.supplyCustomsName = item.supplyCustoms.length?item.supplyCustoms[0].customName:"";
      this.showPop = true;
    },
    delCus() {
