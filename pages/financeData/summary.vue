      ...
      <view style="overflow:auto;height: 100%" v-if="current === 0">
        <view class="summary">
          <view class="summary-item">
            ...
            <view class="summary-content">￥{{summaryData.accumulateSettleAmount}}</view>
          </view>
          <view class="summary-item">
            <view class="summary-title">上期末已付金额</view>
            <view class="summary-content">￥{{summaryData.upperPayAmount}}</view>
          </view>
          <view class="summary-item">
            <view class="summary-title">本期支付金额</view>
            <view class="summary-content">￥{{summaryData.payAmount}}</view>
          </view>
          <view class="summary-item">
            <view class="summary-title">累计已付金额</view>
            <view class="summary-content">￥{{summaryData.accumulatePayAmount}}</view>
          </view>
        </view>
        <view class="cart2">
        <view class="cart2-title">各单位已付金额占比</view>
        ...
        <table v-if="showList.length">
          ...
          <tbody>
            <tr v-for="(item, index) in showList" :key="index">
              ...
              <td>{{ item.accumulateSettleAmount }}</td>
              <td>{{ item.upperPayAmount }}</td>
              <td>{{ item.payAmount }}</td>
              <td>{{ item.accumulatePayAmount }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods: {
    ...
    financePie() {
      this.$api.financePie({ beginDate: this.endTime }).then((res) => {
        if (res.code == 200) {
            this.summaryData =res.data
            let arr = res.data.financePieListVos.map(item=>({ value: item.accumulatePayAmount, name: item.name }))
            if(arr.length){
                this.option = {
                tooltip: {
                  trigger: "item",
    ...
    searchFinancePage() {
      ...
      this.$api.searchFinancePage(data).then((res) => {
        if (res.code === 200) {
            if(data.pageNum==1){
    ...
    tdClick(row) {
      console.log(row);
      uni.navigateTo({ url: `/pages/financeData/financeDetail?beginTime=${this.endTime}&endTime=${this.nowData}&row=${JSON.stringify(row)}` })
    },
    openCale(type, date) {
