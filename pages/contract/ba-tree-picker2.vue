								...
								<view class="tree-flex " @tap.stop="_onItemSelect(item, index)">
									...
									<view class="item-check" v-if="selectParent ? true : item.isLastLevel">
										...
										<view class="item-check-yes" v-if="item.checkStatus == 1"
											:class="{ radio: !multiple, disabled: disabled||item.disabled }"
										...
										<!-- 0没选中状态 -->
										<view class="item-check-no" v-else :class="{ radio: !multiple, disabled: disabled||item.disabled }"
...
export default {
	...
	methods: {
		...
		_formatTreeData(list = [], level = 0, parentItem, isShowChild = true) {
			...
			list.forEach(item => {
				...
				let itemT = {
					...
					designQuantities:item.designQuantities,
					disabled:(this.disabledList.includes(item[this.valueKey])||item.whetherOccupy==0||item.isChooseByArea)?true:false
				};
		...
		_onItemSelect(item, index) {
			...
			this.treeList.forEach((item, index) => {
				if (item.checkStatus === 2) {
					arr.push({id:item.id,designQuantities:item.designQuantities});
				}
			});
