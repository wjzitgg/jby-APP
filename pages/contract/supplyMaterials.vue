        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in details.deductions" :key="index">
              ...
              <td>{{rowData.menuCodeData.states==1? item.supplyPrice:"***"}}</td>
              <td>{{rowData.menuCodeData.states==1?( item.supplyNum * item.supplyPrice):"***"}}</td>
              <td>{{ item.remark }}</td>
            </tr>
          </tbody>
        </table>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in details.noDeductions" :key="index">
              ...
              <td>{{ item.fkUnitName }}</td>
              <td>{{ item.supplyNum + '%'}}</td>
              <td>{{rowData.menuCodeData.states==1? item.excessPrice:"***" }}</td>
              <td>{{ item.remark }}</td>
            </tr>
          </tbody>
        </table>
...
export default {
  ...
  methods:{
    ...
    init(){
      this.$api.contractSupplyMaterialSearch({contractId:this.rowData.pkId,materialName:this.materialName}).then(res=>{
        if(res.code == 200){
