        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in details" :key="index">
              ...
              <td>{{ item.unitName }}</td>
              <td v-if="['1', '2', '4'].includes(contractType)">
                {{
                  item.inventoryCodeName == "费用类清单" ? 1 : item.quantities
                }}
              </td>
              <td>
                {{
                  item.inventoryCodeName == "费用类清单" ? 1 : item.contractNum
                }}
              </td>
              <td>
                {{
                  menuCodeData.states == 1
                    ? item.inventoryCodeName == "费用类清单"
                      ? item.amount
                      : item.price
                    : "***"
                }}
...
export default {
  ...
  onLoad(item) {
    ...
    // if (this.rowData.menuCodeData == undefined) {
    //   this.rowData.menuCodeData = {
    //     states: 1
    //   }
    // }
    this.initList();
  ...
  methods: {
    ...
    initList() {
      this.$api
        .contractDetailsByContractId({ contractId: this.rowData.pkId })
        .then((res) => {
          if (res.code == 200) {
            console.log(res);
