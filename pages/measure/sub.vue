<template>
  <view class="wrapper">
    <u-navbar leftText="分包计价" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
    <view class="sticky">
      ...
      <view class="search">
        <view class="search-item">
          <easy-select ref="easySelect" size="mini" class="easySelect" :value="nowCus" @selectOne="selectOne"
            :options="customList"></easy-select>
        </view>
      </view>
          ...
          <view @click="detail(item)" class="item">
            <view class="df mb-20">
              ...
              <view class="tags">
                ...
                <!-- <view v-if="item.settleStatus===0" class="tag default">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===1" class="tag waring">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===2" class="tag primary">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===3" class="tag error">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===4" class="tag default">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===5" class="tag waring">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===6" class="tag primary">{{item.settleStatusStr}}</view>
                <view v-if="item.settleStatus===7" class="tag default">{{item.settleStatusStr}}</view> -->
              </view>
            </view>
            <view class="money">
  ...
  methods: {
    ...
    searchSettleStates() {
      this.$api.searchSettleStates().then(res => {
        if (res.code === 200) {
          this.tabList = [{ name: "全部", value: "" }, ...res.data.map(item => ({ name: item.key, value: item.name }))]
        } else {
          uni.showToast({
            title: res.msg,
            icon: "none",
    ...
    findCustomByUserId() {
      this.$api
        ...
        .then((res) => {
          if (res.code === 200) {
            this.customList = [{ value: '', label: '全部' }, ...res.data.map(item => ({ ...item, label: item.customName, value: item.pkId }))];
          } else {
            uni.showToast({
              title: res.msg,
              icon: "none",
    ...
    searchSettingPage(resh) {
      ...
      let data = {
        ...
        pageSize: 20,
        beginTime: this.beginTime == undefined ? "" : this.beginTime,
        endTime: this.endTime == undefined ? "" : this.endTime,
        fkOrgId: this.user.orgType === 5 ? "" : uni.getStorageSync("nowOrgId"),
        settleStatus: this.settleStatus ==undefined?"":this.settleStatus,
        settleType: 2,
        settleOrgIds:this.cusId
      };
    ...
    detail(item) {
      console.log(item);
      uni.navigateTo({ url: `/pages/measure/settingDetail?type=2&sendType=${[0,7].includes(item.settleStatus) ? 1 : 2}&pkId=${item.pkId}&settleType=2` });
      // if(item.settleStatus<4){
      //      uni.navigateTo({ url: "/pages/measure/settingDetail?type=2&row="+JSON.stringify(item) });
      //  }else{
      //      uni.navigateTo({ url: "/pages/measure/sealApporval?type=2&row="+JSON.stringify(item) });
      //  }
    },
    scrollTolower() {
    ...
    selectOne(e) {
      console.log(e);
      this.nowCus = e.options.label;
      this.cusId = e.options.value;
      this.searchSettingPage()
    },
    cleanDate(e) {
      console.log(e);
