<template>
  <view class="wrapper">
    ...
    <view v-if="list1.contractChapters&&list1.contractChapters.length">
      <u-tabs class="tabList bgw scroll-tabs" :list="tabList" :activeStyle="{color: 'rgba(32, 52, 87, 1)'}" :inactiveStyle="{color: 'rgba(32, 52, 87, 0.6)'}" :current="current" @change="currentChange" :scrollable="true"></u-tabs>
      ...
      <view class="cart2">
        <view class="cart2-title fw">清单结算占比</view>
        <echart :option="option" :width="662" :height="454" class="mb-10" v-if="echartShow"></echart>
        <view class="noEchart" v-else>
          <!-- <view class="cart2-title fw">清单结算占比</view> -->
          <u-empty style="height: calc(100% - 32rpx)" mode="data" text="暂无已结算工程清单" icon="/static/image/noSettleData.png"></u-empty>
        </view>
      </view>
