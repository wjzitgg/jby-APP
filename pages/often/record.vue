        ...
        <view class="list1" v-if="current < 2 && showList.length">
          <view class="list-item" v-for="item in showList" :key="item.pkId">
            ...
            <view
              ...
              >结算周期：{{ item.beginTime }}~{{ item.endTime }}</view
            >
            ...
            <view
              class="state"
              :class="{ 'st-blue': item.confirmStatus !== 1 }"
              @click="setting(item)"
              >{{ item.confirmStatus === 1 ? "已确认" : "待确认" }}</view
            >
          </view>
        </view>
        <!-- 结余列表 -->
        <view class="list2" v-if="current == 2 && showList.length">
          ...
          >
            ...
            <view class="df2" v-if="userInfo.teamType !== 1">
              <view class="iclass">所在班组：{{ item.className }}</view>
              <view class="principal">工资负责人：{{ item.managerName }}</view>
            </view>
            <view class="time"
...
export default {
  ...
  methods: {
    ...
    getSettlementInfo(settlementType) {
      uni.showLoading({ title: "加载中...", mask: true });
      this.$api
        .getSettlementInfo({ settlementType, projectIds: this.checkboxValue })
        .then((res) => {
          console.log(res);
          if (res.code === 200) {
