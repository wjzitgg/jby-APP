          ...
          <view class="list-item" v-for="item in showList" :key="item.pkId">
            ...
            <view class="item-bottom">
              ...
              <view class="df" v-if="userInfo.teamType !== 1">
                <view class="iclass">所在班组：{{ item.teamName }}</view>
                <view class="principal" >{{ current === 1 ? "发放" : "结算" }}负责人：{{ item.userName }}</view
                >
              </view>
            </view>
            <view
        ...
        <view class="list2" v-if="current == 2 && showList.length">
          ...
          >
            <view class="item-head">
              ...
              <view
                class="state"
                :class="{ 'st-red': item.settlementAmount - item.grantAmount }"
                >{{
                  item.settlementAmount - item.grantAmount ? "未结清" : "已结清"
                }}</view
              >
            </view>
            ...
            <view class="time"
              ...
              {{ item.resignationTime ? item.resignationTime : "至今" }}</view
            >
            <view class="setting-money"
              >结算金额：￥{{ item.settlementAmount }}</view
            >
            <view class="item-bottom">
              <view class="send-money"
                >已发放金额：￥{{ item.grantAmount }}</view
              >
              <view
                class="surplus-money"
                v-if="item.settlementAmount - item.grantAmount > 0"
                >结余金额：￥{{
                  item.settlementAmount - item.grantAmount
                }}</view
              >
            </view>
          </view>
...
export default {
  ...
  methods: {
    ...
    getSurplusInfo() {
      ...
      this.$api
        .getSurplusInfo({ projectIds: this.checkboxValue })
        .then((res) => {
          console.log(res);
          if (res.code === 200) {
    ...
    getSettlementInfo(settlementType) {
      uni.showLoading({ title: "加载中...", mask: true });
      this.$api 
        .getSettlementInfo({pageNum:1,pageSize:10000, settlementType, projectIds: this.checkboxValue,...this.inpDate,objectType:this.userInfo.teamType?1:2 })
        .then((res) => {
          console.log(res);
          if (res.code === 200) {
