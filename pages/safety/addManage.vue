    ...
    <view class="content">
      <view>
        <view class="add-inputs">
          <view class="inputs-label">记录类型</view>
          <view class="inputs-content select" @click="typeShow=true">
            <view class="name">{{ inspectTypeName }}</view>
...
export default {
  ...
  onLoad(item) {
    ...
    if (this.rowData.type == 1) {
      ...
      this.inspectType=this.rowData.inspectType
      this.inspectTypeName=this.rowData.inspectTypeName
      this.receivingType = {
  ...
  methods: {
    ...
    typeconfirm(e){
      if(e.value[0]){
        this.inspectType = e.value[0].inspectType;
        this.inspectTypeName = e.value[0].inspectTypeName;
      }
      this.typeShow = false;
    },
    empty(){
