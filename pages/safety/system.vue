<template>
	<view class="wrapper">
		<u-navbar leftText="安全体系" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
		<!-- 头部tabs -->
		<view class="sticky">
		...
		<view class="content">
			...
			<view class="pri-img" v-if="current === 0">
				<view v-if="fileData1.length" style="position: relative;">
					...
					<view style="position: absolute;top:6px;right:15px;color:red" name="close" @click="openLinkMod(fileDataId1)"  size="10">X</view>
				</view>
				<!-- <u-empty icon="/static/image/noData.png" v-else></u-empty> -->
				<view v-else class="pri-img-upload">
					<u-icon name="photo-fill" color="#ccc" size="150"></u-icon>
					<lsj-upload v-if="$auth('safety:system:add')" style="position: fixed;bottom: 0px;width: 100%;height: 47px;" ref="lsjUpload1" childId="upload2" :width="width" :height="height" :option="option"
						...
						<view v-if="$auth('safety:system:add')" class="upload-btn btn">上传文件</view>
					</lsj-upload>
				</view>
			</view>
			<view class="pri-img" v-if="current === 1">
				<view v-if="fileData2.length" style="position: relative;">
					...
					<view  style="position: absolute;top:6px;right:15px;color:red" name="close" @click="openLinkMod(fileDataId2)" >X</view>
				</view>
			...
			<view class="" v-if="current === 2">
				<view class="list" v-if="fileData3.length > 0">
					<view class="list-item" v-for="(item, index) in fileData3" :key="index" >
						<u-icon name="file-text-fill" color="#d73434" size="28" class="file-icon" @click="previewFile(item.enclosureUrl)"></u-icon>
						<text @click="previewFile(item.enclosureUrl)">{{ item.enclosureName }}</text>
						<u-icon style="position: absolute;top:50%;right:20rpx;transform: translateY(-50%);" name="close" @click="openLinkMod(item.enclosureUrl)" color="red" size="10"></u-icon>
					</view>
				</view>
				<!-- <u-empty icon="/static/image/noData.png" v-else></u-empty> -->
				<view  class="pri-img-upload">
			...
			<view class="pri-img" v-if="current === 4">
				<view v-if="fileData5.length" style="position: relative;">
					...
					<view style="position: absolute;top:6px;right:15px;color:red" name="close" @click="openLinkMod(fileDataId5)" color="red" size="10">X</view>
				</view>
				<!-- <u-empty icon="/static/image/noData.png" v-else></u-empty> -->
				<view v-else class="pri-img-upload">
					<u-icon name="photo-fill" color="#ccc" size="150"></u-icon>
...
export default {
	...
	methods: {
		...
		getData() {
			...
			this.$api.getsafetImg({ hierarchyType: 2,fkOrgId: this.user.orgType === 5 ? "" : uni.getStorageSync("nowOrgId"), }).then(res => {
				if (res.code === 200) {
					console.log(res);
