    ...
    <view class="content">
      <view>
        <!-- <view>
          ...
          <u-picker
            :show="subentryShow"
            :columns="[subentryList]"
            keyName="label"
            @cancel="subentryClose"
            @confirm="subentryconfirm"
          ></u-picker>
        </view> -->
        ...
        <!-- <view>
          <view class="form-title">描述</view>
          <u--textarea
            v-model="rowData.describeContent"
            placeholder="请输入内容"
          ></u--textarea>
        </view> -->
        <view class="add-inputs">
      ...
      >
        <view class="btns">
          <u-button class="btn-iem" text="取消" @click="download" style="background-color:#eeee"></u-button>
        </view>
        <view class="btns">
...
export default {
  ...
  onLoad(item) {
    ...
    if (this.rowData.type == 1) {
      ...
      this.editTitle = "新增上级安全检查";
    } else {
      console.log(this.rowData);
      this.editTitle = "编辑上级安全检查";
  ...
  methods: {
    ...
    subentryconfirm(e) {
      ...
      if( this.subentryData.label== "建设单位(业主方)"){
        ...
        data.type = 3
      }else if(this.subentryData.label== "其他"){
        data.type = 4
    ...
    submit() {
      ...
      if (this.editTitle == "新增上级安全检查") {
        this.$api.firstQualityAdd(data).then((res) => {
          if (res.code == 200) {
            // uni.navigateTo({
            //   url: "/pages/safety/check?",
            // });
            let pages = getCurrentPages()
              let prevPage = pages[pages.length - 2]; // 上一页面实例
              prevPage.$vm.resh()
