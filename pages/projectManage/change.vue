<template>
	<view class="wrapper">
		...
		<view class="sticky">
			...
			<view class="search">
				<view class="search-input">
					<u-input placeholder="请输入变更项目名称" border="none" v-model="inpDate.changesName" maxlength="50">
						<view slot="suffix">
		...
		<view class="content">
			<u-list @scrolltolower="scrollTolower" class="u-list" :height="'calc(100vh - 268rpx)'" v-if="list.length">
				<u-list-item v-for="(item, index) in list" :key="index">
					<view class="item" @click="detail(item)">
						...
						<view class="content">
							...
							<view class="detail mb-20">变更类型：{{ item.fkWorkflowTemplateName }}</view>
							<view class="detail mb-20">发起人：{{ item.createUserName }}</view>
							<view class="detail">申请日期：{{ item.createTime }}</view>
						</view>
						<view class="tags">
							<!-- 结算状态，0：草稿   1：审批中  2：已完成 3: 已拒绝,4:待签章，5：签章中，6：签章已完成-->
							<view v-if="item.changesStatus === 0" class="tag default">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 1" class="tag waring">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 2" class="tag primary">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 3" class="tag error">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 4" class="tag primary">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 5" class="tag waring">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 6" class="tag primary">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 7" class="tag default">{{item.changesStatusStr}}</view>
							<view v-if="item.changesStatus === 10" class="tag default">{{item.changesStatusStr}}</view>
						</view>
					</view>
				</u-list-item>
			</u-list>
			<u-empty style="height: calc(100vh - 268rpx)" mode="data" text="暂无数据" icon="/static/image/noData.png" v-else>
			</u-empty>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeStatsQuery() {
				this.$api.changeStatsQuery2().then(res => {
					if (res.code === 200) {
						this.tabList = [
							{ name: "全部", value: "" },
							...res.data.map(item => ({
			...
			getChangeType() {
				this.$api.getChangeType2({ fkBidProjectId: this.user.orgType !== 4 ? "" : uni.getStorageSync("nowOrgId") }).then(
					res => {
						if (res.code === 200) {
							this.typeList = [
								{ label: "全部", value: "" },
			...
			getCreateUserList() {
				this.$api.getCreateUserList2().then(res => {
					if (res.code === 200) {
						this.userList = [
							{ label: "全部", value: "" },
							...res.data.map(item => ({
			...
			detail(item) {
				uni.navigateTo({
					url: `/pages/change/changeDesignDetail?sup=1&pkId=${item.pkId}&type=${item.changesStatus==0 ? 1 : 2}`,
				});
			},
			scrollTolower() {
