<template>
  <view class="wrapper">
    ...
    <view class="sticky">
      <u-tabs :activeStyle="{color: 'rgba(32, 52, 87, 1)'}" :inactiveStyle="{color: 'rgba(32, 52, 87, 0.6)'}" class="tabList" :list="tabList" :current="current" @change="currentChange" :scrollable="true"></u-tabs>
          ...
          <view @click="detail(item)" class="item">
            <view class="df mb-20">
              ...
              <view class="tags">
                ...
                <view v-if="item.settleStatus===4" class="tag primary">{{item.settleStatusStr}}</view>
...
export default {
  ...
  methods: {
    ...
    detail(item) {
      console.log(item);
      //settleStatus 0：草稿 1：审批中 2：已完成 3: 已拒绝,4:待签章，5：签章中，6：签章已完成
      uni.navigateTo({ url: "/pages/measure/settingDetail?type=2&sendType=2&pkId=" + item.pkId });
    },
    currentChange(e) {
