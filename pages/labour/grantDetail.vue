  ...
  <view class="wrapper">
    ...
    <view class="content">
      ...
      >
        ...
        <u-form-item label="结余金额:" borderBottom v-if="!priview">
          <u-input v-model="settlementAmount" border="bottom" disabled>
            <template slot="suffix">元</template>
          </u-input>
        </u-form-item>
        <u-form-item label="本期发放金额:" borderBottom>
          <u-input v-model="form.settlementAmount" type="number" border="bottom" :disabled="priview" @input="settingChange">
            <template slot="suffix">元</template>
          </u-input>
        </u-form-item>
        <u-form-item label="上期末发放金额:" borderBottom>
          <u-input v-model="form.frontSettlementAmount" border="bottom" disabled>
            <template slot="suffix">元</template>
          </u-input>
        </u-form-item>
        <u-form-item label="本期末发放金额:" borderBottom>
          <u-input v-model="form.afterSettlementAmount" border="bottom" disabled>
            <template slot="suffix">元</template>
          </u-input>
        </u-form-item>
        <u-form-item label="发放日期:" borderBottom @click="openCal">
          <u-input
        ...
        <u-form-item label="确认时间:" borderBottom v-if="priview">
      ...
      <view class="approve-files">
        <view class="approve-files-head">
          ...
          <lsj-upload
            ...
            v-if="!priview&&!pickerShow&&!modelShow"
          >
...
export default {
  ...
  methods: {
    ...
    terminalAmountByTeamId(teamId) {
      this.$api.terminalAmountByTeamId({ teamId, settlementType: 2 }).then((res) => {
          if (res.code === 200) {
    ...
    btnOk(confirmStatus) {
      let data = { ...this.form, objectType: 1, confirmStatus,settlementType:2 };
      data.voucherUrl=JSON.stringify(this.fileList)
      if(!data.fkTeamId){
        return uni.showToast({title:'请选择班组',icon:"none"})
      }
      if(!data.settlementAmount){
        return uni.showToast({title:'请填写本期发放金额',icon:"none"})
      }
      if(data.settlementAmount>this.settlementAmount){
        return uni.showToast({title:'发放金额不可超过结余金额',icon:"none"})
      }
      if(!data.settlementTime){
        return uni.showToast({title:'请选择本期发放时间',icon:"none"})
      }
      uni.showLoading({mask:true})
      if (this.type === 1) {
     ...
     edit() {
      this.labourTeamSearch();
      this.terminalAmountByTeamId(this.form.fkTeamId)
      this.priview = false;
    },
    goInfo(item) {
      uni.navigateTo({
    ...
    settingChange(value) {
      ...
      if (this.timeout) {
			...
			this.timeout = setTimeout(() => {
				if(Number(value) > this.settlementAmount){
        this.form.settlementAmount = this.settlementAmount
      }
				this.form.afterSettlementAmount = this.form.frontSettlementAmount - 0 + (this.form.settlementAmount - 0);
			}, 200);
    },
