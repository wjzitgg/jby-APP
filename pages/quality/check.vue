...
<template>
	<view class="wrapper">
		<u-navbar leftText="上级质量检查" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
		<view class="content">
			<!-- ======================== -->
			<!-- <template>
		...
		<u-popup :show="itemShow" mode="bottom" :round="20" @close="closePops" @open="openPops">
			<view class="item-edit">
				...
				<view>
					<u--form labelPosition="left" :model="contractDetails" class="item-edit-form">
						<!-- <u-form-item label="上级单位:" prop="userInfo.name" borderBottom labelWidth="80" ref="item1">
          ...
          ">记 录:{{ contractDetails.content }}</view>
				<view style="background: #f6f6fc; height: 5px; margin-top: 20px"
					v-if="contractDetails.enclosures!=null && contractDetails.enclosures.length != 0"></view>
				<view style="padding: 8px" v-if="contractDetails.enclosures!=null && contractDetails.enclosures.length != 0">
					<view>附件</view>
				...
				<view style="padding: 8px" v-else>
					<view>附件: 无附件</view>
					<!-- <view class="file-set-list" >无附件</view> -->
				</view>
				<view style="background: #f6f6fc; height: 5px; margin-top: 20px"></view>
		...
		<view type="primary" class="btn" @click="add" v-if="user.orgType == 5">新增上级质量检查</view>
	...
	export default {
		...
		methods: {
			...
			deleteItem() {
				...
				this.$api.firstQualityDelete({ pkId: this.contractDetails.pkId })
					.then(res => {
						uni.hideLoading();
						if (res.code == 200) {
							this.init();
			...
			init() {
				console.log("this.identifying -----> ", this.$refs.solarClender);
				let data = {
					pageNum: 1,
					pageSize: 200,
					hierarchyType: 1,
