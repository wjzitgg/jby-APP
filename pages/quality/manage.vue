...
<template>
  <view class="wrapper">
    <u-navbar leftText="质量日常管理" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
    <view class="content">
      <solarClender @change="change" ref="solarClender" :markDays="markDays" @onDayClick="clickInit"></solarClender>
      <view class="search">
        <view class="left">
          ...
          <view class="search-item" @click="receiverClick">
            ...
            <u-icon v-if="receiverDataList.itemName.length != 0" @click.native.stop="empty" name="trash-fill" class="icons icon" color="#2a82e4" size="16"></u-icon>
    ...
    <u-popup :show="itemShow" mode="bottom" :round="20" @close="closePop" @open="openPops">
      <view class="item-edit">
        <view class="tem-edit-title">
          <text class="tem-edit-titletext"> {{ contractDetails.itemName }}</text>  
        ...
        <view>
          <u--form labelPosition="left" :model="contractDetails" class="item-edit-form">
            ...
            <u-form-item label="记录:" prop="userInfo.sex" borderBottom ref="item1" labelWidth="80" >
              <view style="max-height:150px;overflow: auto;">{{ contractDetails.describeContent }}</view>
            </u-form-item>
          </u--form>
        </view>
        ...
        <view style="padding: 8px">
          <view>附件: <text v-if="contractDetails.enclosures ==null || contractDetails.enclosures.length == 0">无附件</text></view>
        ...
        <view class="popup-footer" style="justify-content: space-between; width: 100%">
          <view class="btns">
            <u-button class="btn-iem" text="删除" @click="deleteItem" v-if="contractDetails.deleteStatus"></u-button>
          </view>
          <view class="btns">
    ...
    <prviewPop class="prview-pop" :previewShow="previewShow" :previewUrl="previewUrl" @close="closePre"></prviewPop>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    searchOther() {
      this.$api.searchOther({ hierarchyType: 1, fkOrgId: uni.getStorageSync("nowOrgId") ,yearMonth:this.yearMonth}).then(res => {
        if (res.code == 200) {
          // this.yearList = []
    ...
    add() {
      ...
      uni.navigateTo({
        url: "/pages/quality/addManage?row=" + JSON.stringify(data),
      });
    },
    itemExamine(row) {
