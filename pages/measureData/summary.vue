<template>
	<view class="wrapper">
		<u-navbar leftText="计量汇总" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
		<view class="search">
			...
			<u-tabs :list="list" :current="current" @change="currentChange" :activeStyle="{ color: 'rgba(32, 52, 87, 1)' }"
			...
			<view style="overflow:auto;height: 100%" v-if="current === 0">
				<view class="summary">
					<view class="summary-item">
						<view class="summary-title">合同金额</view>
						<view class="summary-content">￥{{summaryData.contractAmount}}</view>
					</view>
					<view class="summary-item">
						<view class="summary-title">上期末结算金额</view>
						<view class="summary-content">￥{{summaryData.lastSettleAmount}}</view>
					</view>
					<view class="summary-item">
						<view class="summary-title">本期结算金额</view>
						<view class="summary-content">￥{{summaryData.currentSettleAmount}}</view>
					</view>
					<view class="summary-item">
						<view class="summary-title">累计结算金额</view>
						<view class="summary-content">￥{{summaryData.settleAmount}}</view>
					</view>
				</view>
				<view class="cart2">
					<view class="cart2-title">各单位已结算金额占比</view>
					<echart :option="option" :width="702" :height="536" class="mb-10" v-if="echartShow"></echart>
					<view class="noEchart" v-else>
						<u-empty style="height: calc(100% - 32rpx)" mode="data" text="暂无数据"
							icon="/static/image/noSettleData.png"></u-empty>
					</view>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			this.endTime = now;
			this.nowData = now;
			this.searchSettlePage();
			this.getSettleSummaryAmount();
		},
		methods: {
			scrolltolower(e) {
			...
			getSettleSummaryAmount() {
				this.$api.getSettleSummaryAmount({ beginTime: this.endTime }).then(res => {
					if (res.code == 200) {
						this.summaryData = res.data;
						let arr = res.data.settleSummaryPieVos.map(item => ({ value: item.summaryAmount, name: item.unitName }));
						if (arr.length) {
							this.option = {
								tooltip: {
									trigger: "item",
			...
			searchSettlePage() {
				...
				this.$api.searchSettlePage2(data).then(res => {
					if (res.code === 200) {
						if (data.pageNum == 1) {
			...
			tdClick(row) {
				console.log(row);
				uni
					.navigateTo({ url: `/pages/measureData/summaryDetail?fkProjectId=${row.subOrgId}&endTime=${this.nowData}&beginTime=${this.endTime}` });
			},
			openCale(type, date) {
