      ...
      <view style="overflow:auto;height: 100%" v-if="current === 0">
        <view class="summary">
          <view class="summary-item">
            <view class="summary-title">上年度末合同金额</view>
            <view class="summary-content">￥{{summaryData.upperYearAmount}}</view>
          </view>
          <view class="summary-item">
            <view class="summary-title">本年度合同金额</view>
            <view class="summary-content">￥{{summaryData.yearAmount}}</view>
          </view>
          <view class="summary-item">
            <view class="summary-title">累计合同金额</view>
            <view class="summary-content">￥{{summaryData.amount}}</view>
          </view>
        </view>
        <view class="cart2">
        <view class="cart2-title">各单位合同金额占比</view>
        ...
        <table v-if="showList.length">
          ...
          <tbody>
            <tr v-for="(item, index) in showList" :key="index">
              ...
              <td>{{ item.contractOrgName }}</td>
              <td>{{ item.signDuration }}</td>
...
export default {
  ...
  methods: {
    ...
    contractPie() {
      this.$api.contractPie({ engDate: this.endTime }).then((res) => {
        if (res.code == 200) {
            ...
            let arr = res.data.contractPieListVos.map(item=>({ value: item.contractAmount, name: item.name }))
            if(arr.length){
                this.option = {
                tooltip: {
                  trigger: "item",
    ...
    searchContractPage() {
      ...
      this.$api.searchContractPage(data).then((res) => {
        if (res.code === 200) {
            if(data.pageNum==1){
    ...
    tdClick(row) {
      console.log(row);
      uni.navigateTo({ url: '/pages/contractData/contractDetail?projectBidId=' + row.pkId })
    },
    openCale(type, date) {
