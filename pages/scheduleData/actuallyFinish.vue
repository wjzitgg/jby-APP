<template>
  <view class="wrapper">
    <u-navbar leftText="实际完成" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
    <view class="sticky">
      <u-tabs class="tabList" :list="tabList" :activeStyle="{color: 'rgba(32, 52, 87, 1)'}" :inactiveStyle="{color: 'rgba(32, 52, 87, 0.6)'}" :current="current" @change="currentChange" :scrollable="true"></u-tabs>
      ...
      <view class="search-item2" v-if="current === 1">
    ...
    <!-- <view :class="{ pad: !isOk, pad2: isOk }"></view> -->
    <view class="pad"></view>
    <view class="content">
        ...
        <table v-if="list1.length">
          ...
          <tbody>
            <tr v-for="(item, index) in list1" :key="index">
              ...
              <td>{{item.unitName}}</td>
              <td>{{  item.price }}</td>
              <td>{{  item.designQuantities }}</td>
              <td>{{  item.designAmount }}</td>
              <td>{{  item.completeQuantities }}</td>
      ...
      <view class="table_detail table_empty table_height" v-if="current === 1">
...
export default {
  onLoad(options) {
    ...
    this.fkBidProjectId=options.id
  ...
  methods: {
    ...
    findWorkAreasByUserId() {
      this.$api
        ...
        .then((res) => {
          if (res.code === 200) {
            this.areaList = [{ value: '', label: '全部' }, ...res.data.map(item => ({ ...item, label: item.areaName, value: item.pkId }))];
          } else {
            uni.showToast({
              title: res.msg,
              icon: "none",
