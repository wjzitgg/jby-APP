...
export default {
    ...
    data(){
        return{
            ...
            orgType:"",
            linkList:[],
            orgTypeList: ["系统运营商", "系统代理商", "建设单位", "监理公司", "施工单位", "项目部", "供应商", "分包商", "劳务工人", "设计院",'施工单位集团总公司',"政府监管单位",'建设单位集团总公司'],
            emptyShow:false,
            searchFocus:false
        }
    },
    methods:{
        ...
        searchLinkList() {
			...
			this.loading = true;
			this.$api.searchSuperiorOrgByLinkPhone(data).then(res => {
				this.loading = false;
				if (res.code === 200) {
                    this.emptyShow=true
        ...
        updateRelationById(orgId) {
			...
			this.$api.bindingSuperior(orgId ).then(res => {
				uni.hideLoading();
				if (res.code === 200) {
                    this.resh()
