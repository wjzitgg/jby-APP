...
<template>
  <view class="wrapper">
    <u-navbar leftText="会议通知" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
    <view class="pad"></view>
    <view class="sticky">
      <view class="search">
        <view class="search-input">
          <u-input placeholder="发布人/会议主题/会议内容"  border="none" v-model="inpDate.keyWord" maxlength="100">
            <view slot="suffix">
    ...
    <view class="content">
      <u-list @scrolltolower="scrollTolower" class="u-list" :height="'calc(100vh - 280rpx)'">
        <u-list-item v-for="(item, index) in list" :key="index">
          <view class="item" @click="detail(item)">
            ...
            <view class="content">
              <view class="name">{{ item.meetingTitle }}</view>
              <view class="details mb-20">会议时间：{{ item.meetingTime }}</view>
              <view class="detail mb-20">发布人：{{ item.createUserName }}</view>
    ...
    <view class="pdb"></view>
    <view class="btn" @click="addBtn" v-if="$auth('assess:notice:add')">新增</view>
...
export default {
  ...
  methods: {
    ...
    detail(item) {
      uni.navigateTo({ url: '/pages/assess/meetingDetails?type=1&obj=' + JSON.stringify(item) })
    },
    scrollTolower() {
    ...
    addBtn() {
      uni.navigateTo({ url: '/pages/assess/meetingDetails?type=0' })
    }
  },
};
</script>
