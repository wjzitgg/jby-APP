<template>
	<view class="wrapper">
		<u-navbar leftText="商务用户" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
		<view class="pad"></view>
		<view class="sticky">
			...
			<view class="search">
				<view class="search-input">
					<u-input placeholder="请输入客户名称或者手机号码" border="none" v-model="inpDate.keyWord">
						<view slot="suffix">
			...
			<u-list @scrolltolower="scrollTolower" class="u-list" :height="'calc(100vh - 268rpx)'">
				<u-list-item v-for="(item, index) in list" :key="index">
					<view class="item" @click="detail(item)">
						...
						<view class="content">
							<view class="name">{{ '【'+item.orgTypeName+'】'+item.accountName }}</view>
						...
						<view class="tags">
							...
							<view class="tag"
								...
								{{item.approveStatus==0?'审批中':item.approveStatus==1?'审批通过':item.approveStatus==2?'审批不通过':"" }}
							</view>
						</view>
					</view>
				</u-list-item>
...
<script>
	export default {
		...
		methods: {
			...
			detail(item) {
				uni.navigateTo({
					url: "/pages/waiting/renewayApproval?type=0&row=" + JSON.stringify({
						fkBusinessId: item
							.fkRenewalId,
						...item
					})
