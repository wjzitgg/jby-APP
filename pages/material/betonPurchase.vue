<template>
	<view class="wrapper">
		...
		<view class="btn" v-if="$auth('material:betonPurchase:add')" @click="addProcess">新增采购计划单</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				searchData: {
					pageNum: 1,
					pageSize: 20,
					purchaseTypeCode: "purchase_concrete", //混凝土
					purchaseCode: "",
					orderCode: "", // 采购编号
					beginTime: "",
					endTime: "",
		...
		methods: {
			...
			detail(item) {
				uni.navigateTo({
					url: "/pages/material/orderDetails?num=1&pkId=" + item.pkId
				});
			},
			getData() {
			...
			addProcess(id) {
				uni.navigateTo({
					url: "/pages/material/newPlannedorder?num=1&type=1"
				});
			},
			// 获取发起人筛选列表
			sponsorList() {
				...
				this.$api.getClient({ customType: 3, supplierType: 2, projectBidId }).then(res => {
					if (res.code === 200) {
