<template>
  <view class="wrapper">
    <u-navbar leftText="分包商材料申请单" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
    <view class="sticky">
        ...
        <u-list-item v-for="(item, index) in list" :key="index">
          <view class="item" @click="detail(item)">
            ...
            <view class="tags">
              ...
              <view v-else-if="item.changesStatus === '已完成'" class="tag waring">{{ item.applyCode }}</view>
              <view v-else class="tag waring">{{ item.applyCode }}</view>
            </view>
          </view>
        </u-list-item>
      </u-list>
  ...
  methods: {
    ...
    searchChangesDesigPage() {
      let data = {
        ...
        createUser:this.inpDate.createUser
      };
    ...
    detail(item) {
      ...
      if (item.changesStatus < 4) {
        uni.navigateTo({
          url: "/pages/material/apply/subcontractorDetails?row=" + JSON.stringify(items),
        });
      } else {
        uni.navigateTo({
          url: "/pages/material/apply/subcontractorDetails?row=" + JSON.stringify(items),
