<template>
  <view class="wrapper">
    ...
    <view class="content">
      ...
      <view class="table_detail table_empty" v-show="current == 1" style="margin-top: 2px">
        <table>
          ...
          <tbody>
            <tr
              v-for="(item, index) in details.orderApplyMaterialDetails"
              :key="index"
            >
              <td>{{ index + 1 }}</td>
    ...
    <view class="box-btn">
      <u-button type="error"  v-if="details.isHandle ==1" text="驳回" @click="redact"></u-button>
      <u-button type="primary" v-if="details.isHandle ==1" text="处理" @click="submit(2)"></u-button>
    </view>
  </view>
</template>
  ...
  methods: {
    ...
    submit(val) {
      ...
      this.$api.updateApplyCodeByBusinessType(data).then((res) => {
        if (res.code == 200) {
          // uni.navigateTo({
          //   url: "/pages/material/apply/subcontractor?",
          // });
          // uni.showToast({ icon: "none", title: res.msg });
          uni.showToast({ icon: "none", title: res.msg });
            setTimeout(() => {
