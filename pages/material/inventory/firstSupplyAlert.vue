<template>
  <view class="wrapper">
    <u-navbar leftText="甲供预警" bgColor="rgb(0 0 0 / 0%)" leftIconColor="#fff" :autoBack="true"></u-navbar>
    <view class="content" style="position: relative">
      <view class="search-box">
        ...
        <view class="right">
          <view class="search-input"><u-input border="none" v-model="warehouseName" maxlength="50">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="(item, index) in list" :key="index">
              ...
              <td>{{ item.stockNum }}</td>
              <td>{{ item.restrictSupplyNum ? item.restrictSupplyNum : "-" }}</td>
              <td>{{ item.supplyNum }}</td>
              <td style="color:red">{{ item.excessNum }}</td>
              <td style="color:red">{{ item.excessDeductAmount }}</td>
            </tr>
          </tbody>
        </table>
        <u-empty mode="data" text="没有更多了" icon="/static/image/tableNoMore.png"></u-empty>
      </view>
      <view type="primary" class="btn" @click="add" v-if="$auth('material:inventory:edit')">新增预警项</view>
    </view>
    <u-popup :show="showLinkPop" round="10" mode="bottom">
      <view class="showLinkPop">
        ...
        <view class="content">
          <view>
            <view class="form-title">分包商</view>
            ...
            <u-picker :show="warehouseShow" :columns="warehouseList" keyName="name" @cancel="warehouseClose"
          ...
          <view>
            ...
            <u--input placeholder="请输入内容" border="none" v-model="rowData.alertUpperNum" customStyle="" type="number"
              class="form-content" clearable></u--input>
          </view>
          <view>
            <view class="form-title">预警下限</view>
            <u--input placeholder="请输入内容" border="none" v-model="rowData.alertDownNum" customStyle="" class="form-content" type="number"
              clearable></u--input>
          </view>
          <view>
            <view class="form-title">现有库存量</view>
            <u--input placeholder="请输入内容" border="none" v-model="rowData.stockNum" customStyle="" disabled
              class="form-content" clearable></u--input>
          </view>
          <view>
            <view class="form-title">限额库存量</view>
            <u--input placeholder="请输入内容" border="none" v-model="rowData.restrictSupplyNum" customStyle="" disabled
              class="form-content" clearable></u--input>
          </view>
          <view>
            <view class="form-title">已供应量</view>
            <u--input placeholder="请输入内容" border="none" v-model="rowData.supplyNum" customStyle="" disabled
              class="form-content" clearable></u--input>
          </view>
          <view class="footer" style="margin-top: 50px" v-if="$auth('material:inventory:edit')">
            ...
            <u-button v-if="rowData.title == '编辑甲供预警'" class="btns" type="error" text="删除" @click="expurgate"></u-button>
            <u-button class="btns" type="primary" @click="preserve" text="保存"></u-button>
          </view>
        </view>
      </view>
    </u-popup>
  ...
  methods: {
    selectMaterials() {
      if(!this.warehouseData.pkId){
        uni.showToast({title:"请选择分包单位",icon:"none"})
        return
      }
      this.getMaterials();
    ...
    // 查物料
    getMaterials() {
      this.$api.getMaterials({projectId:this.user.orgType === 5 ? "" : uni.getStorageSync("nowProId"),customId:this.warehouseData.pkId}).then((res) => {
        if (res.code == 200) {
    ...
    getMaterialsInventory(){
      let data={materialId:this.materialData.pkId,customId:this.warehouseData.pkId}
      this.$api.getMaterialsInventory(data).then(res=>{
        if(res.code===200){
          this.$set(this.rowData,'stockNum',res.data.inventoryNum)
          this.$set(this.rowData,'restrictSupplyNum',res.data.restrictSupplyNum)
          this.$set(this.rowData,'supplyNum',res.data.supplyNum)
        }else{
          uni.showToast({ title: res.msg, icon: "none" });
        }
      })
    ...
    warehouseSearch() {
      let data = {
        ...
        // fkWarehouseId: this.tabList[this.current].pkId,
      };
    ...
    detail(row) {
      ...
      this.warehouseData = {
        name: row.customName,
        pkId: row.fkCustomerId
      }
      this.materialData = {
        fkTypeName: row.materialName,
        pkId: row.fkMaterialId
      }
      this.rowData.title = "编辑甲供预警";
    ...
    preserve() {
      let data = {
        ...
        fkCustomerId: this.warehouseData.pkId, //仓库id
      };

      if (this.rowData.title == "新增甲供预警") {
