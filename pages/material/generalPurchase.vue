	...
	<view class="wrapper">
		...
		<scroll-view class="box" scroll-y="true" @scrolltolower="reachBottom">
			<view :key="current">
					...
					<view class="item" v-for="item in tableData" :key="item.pkId" @click="detail(item)">
						<view class="icons">
							...
							<view class="tags">
								...
								<view v-if="item.purchaseCode === '草稿'" class="tag default">{{ item.purchaseCode }}</view>
								<view v-else-if="item.purchaseCode === '待确认'" class="tag waring">{{ item.purchaseCode }}</view>
								<view v-else-if="item.purchaseCode === '已确认'" class="tag primary">{{ item.purchaseCode }}</view>
								<view v-else-if="item.purchaseCode === '已驳回'" class="tag error">{{ item.purchaseCode }}</view>
								<view v-else-if="item.purchaseCode === '已完成'" class="tag default">{{ item.purchaseCode }}</view>
							</view>
						</view>
						<view class="describe">
				...
				<u-empty v-else class="noData" mode="data" text="暂无数据" icon="/static/image/noData.png"></u-empty>
			</view>
		</scroll-view>
		<!-- 筛选弹框 -->
		...
		<view class="btn" v-if="$auth('material:generalPurchase:add')" @click="addProcess">新增采购计划单</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			detail(item) {
				uni.navigateTo({
					url: "/pages/material/orderDetails?num=0&pkId=" + item.pkId
				});
			},
			getData() {
				...
				if (this.user.orgType !== 5 && this.user.orgType !== 6) {
			...
			addProcess(id) {
				uni.navigateTo({
					url: "/pages/material/newPlannedorder?num=0&type=1"
				});
			},
			// 获取发起人筛选列表
			sponsorList() {
				...
				this.$api.getClient({ customType: 3, supplierType: 1, projectBidId }).then(res => {
					if (res.code === 200) {
