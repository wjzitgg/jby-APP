	...
	<view class="tki-tree">
		...
		<view class="tki-tree-cnt" :class="{ show: showTree }">
			...
			<view class="tki-tree-view">
				<scroll-view class="tki-tree-view-sc" :scroll-y="true"  v-if="treeList.length">
	...
	methods: {
		...
		_renderTreeList(list = [], rank = 0, parentId = [], parents = []) {
			list.forEach(item => {
				...
				if (Array.isArray(item[this.children]) && item[this.children].length > 0) {
					// console.log(item)
					let parentid = [...parentId],
						parentArr = [...parents],
						childrenid = [...childrenid];
					delete parentArr[this.children];
					parentid.push(item[this.idKey]);
					parentArr.push({
						[this.idKey]: item[this.idKey],
						[this.rangeKey]: item[this.rangeKey]
					});
					this._renderTreeList(item[this.children], rank + 1, parentid, parentArr);
				} else {
