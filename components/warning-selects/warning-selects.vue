			...
			<view class="selectShow">
				<view class="tit-top">
					...
					<view class="search-top" v-if="searchShow">
						<u-input placeholder="请输入内容" border="surround" class="feed" v-model="keyWork">
					...
					<view class="tabs-tit" v-if="checkedList.length">
						...
						<text v-for="(item, index) in checkedList" :key="index">{{ item.fkTypeName }}</text>
					</view>
				</view>
					...
					<view>
						...
						<u-list @scrolltolower="scrolltolower" v-if="checkedList.length == 0">
							<u-list-item v-for="(item, index) in treeData" :key="index">
								<u-cell :title="item.fkTypeName" @click="pitchOn(item)"><u-icon v-if="item.children" slot="right-icon" name="arrow-right"></u-icon></u-cell>
							</u-list-item>
						</u-list>
						<!--  -->
						<u-list @scrolltolower="scrolltolower" v-else>
							<u-list-item v-for="(item, index) in treeData2" :key="index">
								<u-cell :title="item.children ? item.fkTypeName : item.fkTypeName" @click="pitchOn(item)">
									<u-icon v-if="item.children" slot="right-icon" name="arrow-right"></u-icon>
								</u-cell>
							</u-list-item>
						</u-list>
					</view>
