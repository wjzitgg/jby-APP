<template>
	<view class="editors">
		...
		<view style="margin-top:5px">
			<mp-html ref="articless" container-style="margin:20px;max-height:400rpx;overflow:auto" :content="content"
...
<script>
	...
	export default {
		...
		mounted() {
			 ...
			 */
			this.$refs.articless.getSrc = (type, value) => {
				return new Promise((resolve, reject) => {
		...
		methods: {
            getText(){
               let text = this.$refs.articless.getContent()
               return text
            },
			// 删除图片/视频/音频标签事件
			remove(e) {
			...
			edit(e) {
				this.checkEditable().then(() => {
                    console.log(e);
					this.$refs.articless[e.currentTarget.dataset.method](e.currentTarget.dataset.param)
				})
			},
			// 插入 head 系列标签
			insertHead() {
				this.checkEditable().then(() => {
					uni.showActionSheet({
						...
						success: res => {
							...
							this.$refs.articless.insertHtml(`<${tagName}>标题</${tagName}>`)
						}
					})
				}).catch(() => {})
			},
			...
			insertTable() {
				this.checkEditable().then(() => {
					...
					this.callback = {
						resolve: () => {
							this.$refs.articless.insertTable(this.rows, this.cols)
						},
						reject: () => {}
					}
				}).catch(() => {})
				...
				this.checkEditable().then(() => {
					uni.showActionSheet({
						...
						success: res => {
							...
							this.$refs.articless.insertHtml(`<pre><code class="language-${lan}">${lan} code</code></pre>`)
						}
					})
				}).catch(() => {})
			},
			// 插入 emoji
			insertEmoji(e) {
				this.$refs.articless.insertHtml(e.currentTarget.dataset.emoji)
				this.closeDialog()
			},
			// 清空编辑器内容
			clear() {
								...
								uni.chooseMessageFile({
									...
									success: res => {
										const content = uni.getFileSystemManager().readFileSync(res.tempFiles[0]
											.path, 'utf8')
										this.$refs.articless.setContent(content)
									}
								})
							}
						}
			...
			save() {
				setTimeout(() => {
					if (this.editable) {
						var content = this.$refs.articless.getContent()
						uni.showModal({
							title: '保存',
							content,
							confirmText: '完成',
