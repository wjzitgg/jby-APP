...
export default {
  ...
  watch:{
    top(val){
      ...
      if(val){
        if(this.type===2){
          if(this.editInfo.y <=80){
            this.editInfo.x=this.index*60+this.index*150+this.padWid/2+40
            this.editInfo.y=20
  ...
  methods: {
    ...
    touchend(e) {
      ...
      if(this.type===1){
        ...
        this.$emit("getPosition", this.editInfo);
        this.editInfo={
          x:this.index*60+this.index*150+this.padWid/2+40,
          y:20
        }
        console.log('ce',this.editInfo);
      }
    },
  },
};
