			...
			<view class="tree-view">
				<scroll-view class="tree-list" :scroll-y="true">
					<block v-for="(item, index) in treeList" :key="index">
						<view class="tree-item" :style="[{ paddingLeft: item.level * 30 + 'rpx' }]"
								...
								<view class="tree-flex " @tap.stop="_onItemSelect(item, index)">
									...
									<view class="item-check" v-if="selectParent ? true : item.isLastLevel">
										...
										<view class="item-check-yes" v-if="item.checkStatus == 1"
											:class="{ radio: !multiple, disabled: disabled }"
										...
										<!-- 0没选中状态 -->
										<view class="item-check-no" v-else :class="{ radio: !multiple, disabled: disabled }"
...
export default {
	...
	methods: {
		...
		_onItemSelect(item, index) {
			...
			if (this.associatedNode) {
				if (item.level == 2 && item.checkStatus == 0) {
					return
				}
				//父节点勾选关联子节点
		...
		_onItemChildSelect(item, index) {
			...
			if (item.childCount && item.childCount > 0) {
				...
				while (index < this.treeList.length && this.treeList[index].level > item.level) {
					...
					console.log('>>>>index1111：', index, 'item：', itemChild.name, '  status：', itemChild
						.checkStatus)
					index++;
				}
			}
		...
		_onItemParentSelect(item, index, oldCheckStatus) {
			...
			if (parentIndex >= 0) {
				...
				this.treeList.forEach(item => {
					...
					if (item.parentId == itemParent.id && item.checkStatus != 0 || itemParent.checkStatus != 0) {
				...
				if (checkStatus) {
					// console.log("3333333333333333xxxxxxxxxxxxxx")
					itemParent.checkStatus = 0;
				}
				this._onItemParentSelect(itemParent, parentIndex, oldCheckStatusParent);
			}
