<template>
  <view class="signBox" @touchstart="touchstart" @touchmove.stop.prevent="touchmove" @touchend.stop.prevent="touchend" :id="id" :style="{ left: editInfo.x+ 'px', top: editInfo.y + 'px' }" style="position: absolute; text-align: center; background: rgba(194, 194, 194, 0.568); width: 60px; height: 30px; " ref="signBox" >
  ...
  methods: {
    ...
    touchmove(e) {
      ...
      if (top < 0) {
        ...
        top=0
        this.editInfo.y = top;
      } else if (top >= (505.2*this.page-30)) {
        top=(505.2*this.page-30)
        this.editInfo.y = top;
      }
